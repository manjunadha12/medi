...
import {
  ...
  AlertTriangle, FileText, Monitor, IndianRupee,
          ...
          <div className="max-w-[1600px] w-full mx-auto flex-1 space-y-6 sm:space-y-10">
            ...
            <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
              <div className="text-left">
                <h1 className={`text-2xl sm:text-3xl lg:text-4xl font-black tracking-tighter uppercase ${theme === 'dark' ? 'text-white' : 'text-slate-800'}`}>Hospital Command Center</h1>
                <p className="text-slate-500 font-bold uppercase text-[8px] sm:text-[10px] tracking-[0.3em] sm:tracking-[0.4em] mt-1">Global management overview for {user?.hospitalName || 'Medi Consult'}</p>
              </div>
              <div className={`px-5 sm:px-8 py-3 sm:py-4 rounded-2xl sm:rounded-[28px] border shadow-sm flex items-center gap-3 sm:gap-4 transition-all duration-500 w-full md:w-auto ${theme === 'dark' ? 'bg-[#0A0A0F] border-white/5' : 'bg-white border-slate-100'}`}>
                 <div className="w-2 sm:w-2.5 h-2 sm:h-2.5 bg-green-500 rounded-full animate-pulse shadow-[0_0_10px_rgba(34,197,94,0.8)]"></div>
                 <span className="text-[8px] sm:text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] sm:tracking-[0.3em]">System Synchronized</span>
              </div>
            </header>
              ...
              {[
                ...
                <div key={i} className={`bg-[#0A0A0A] p-6 sm:p-8 rounded-[32px] border border-white/5 flex flex-col justify-between min-h-[160px] relative overflow-hidden group transition-all duration-500 hover:border-white/10`}>
                  ...
                  <div className="relative z-10 mt-auto">
                     <h3 className="text-2xl sm:text-3xl font-black text-white tracking-tighter leading-none">{item.value}</h3>
                  </div>
