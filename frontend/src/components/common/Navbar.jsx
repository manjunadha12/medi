...
import { Bell, Search as SearchIcon, MoreVertical, Settings, LogOut, ShieldAlert, Sparkles, Pill, CheckCircle, Trash2, X, Loader2, User as UserIcon, ChevronRight } from 'lucide-react';
...
const Navbar = () => {
  const { user, theme, notifications, logout, clearNotification, clearAllNotifications } = useStore();
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(async () => {
      if (searchQuery.length >= 2 && user && (user.role === 'doctor' || user.role === 'admin')) {
        setIsSearching(true);
        try {
  ...
  return (
    <nav className={`h-16 sm:h-20 backdrop-blur-3xl border-b px-4 sm:px-8 flex items-center justify-between sticky top-0 z-[100] transition-all duration-500 ${
      ...
      <div className="flex items-center gap-3 shrink-0 mr-2 sm:mr-6 cursor-pointer" onClick={() => navigate('/')}>
        <img
          src={logoImg}
          alt="Medi"
      ...
      <div className="hidden sm:block flex-1 max-w-xl relative group" ref={searchRef}>
        <div className={`flex items-center gap-3 px-6 py-3 rounded-2xl border transition-all duration-300 ${
          ...
          <input
            type="text"
            placeholder={user?.role === 'patient' ? "Search health knowledge..." : "Search patient name or ID..."}
            value={searchQuery}
      ...
      <div className="flex items-center gap-3 sm:gap-6 shrink-0 relative">
        ...
        >
          ...
          <div className="w-9 h-9 sm:w-11 sm:h-11 bg-gradient-to-tr from-blue-600 to-indigo-600 rounded-xl sm:rounded-2xl flex items-center justify-center text-white font-black text-xs">
        ...
        <div className="relative" ref={menuRef}>
          ...
          {showMenu && (
            <div className={`absolute right-0 mt-3 w-48 rounded-[24px] border shadow-2xl p-2 z-[1000] animate-in fade-in slide-in-from-top-2 duration-200 ${theme === 'dark' ? 'bg-zinc-900 border-white/10 text-white' : 'bg-white border-slate-200 text-slate-800'}`}>
              <button 
                onClick={() => { navigate(user?.role === 'doctor' ? '/doctor/settings' : '/settings'); setShowMenu(false); }}
