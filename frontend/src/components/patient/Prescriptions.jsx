...
import { 
  ...
  Search as SearchIcon, ShieldCheck as ShieldCheckIcon, CheckCircle, Info, ChevronRight, X, Trash2, Loader2, Plus
} from 'lucide-react';
import useStore from '../../store/useStore';
import { scheduleMedicineAlarm, requestNotificationPermission } from '../../utils/notifications';
...
const Prescriptions = () => {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Purge this prescription from node memory? This cannot be undone.")) return;

    setDeletingId(id);
    try {
      await api.delete(`/patients/prescription/${id}`);
  ...
  const addToSchedule = async (medicine) => {
    try {
      const times = [];
      if (medicine.morning) times.push('08:00');
      if (medicine.afternoon) times.push('14:00');
      if (medicine.night) times.push('20:00');

      if (times.length === 0) times.push('09:00'); // Fallback
      ...
      for (const time of times) {
        const payload = {
          name: medicine.name,
          dosage: medicine.dosage || medicine.strength || '1 Tab',
          time: time,
          food: medicine.foodInstruction || 'After Food',
      ...
      toast.success(`${medicine.name} synchronized with Medication Schedule!`);
    } catch (err) {
      console.error(err);
      toast.error("Failed to link medication node.");
    }
  };
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 space-y-10 custom-scrollbar relative z-10 print:p-0 print:m-0 print:bg-white print:text-black">
          ...
          <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/5 pb-8 print:hidden">
            <div>
              <h1 className="text-3xl font-black text-white uppercase tracking-tight">Prescription Terminal</h1>
              <p className="text-blue-500 uppercase text-[10px] font-black tracking-widest mt-1">Verified digital care prescription archives</p>
            </div>
            ...
            <div id="print-area" className="hidden print:block p-10 bg-white text-black font-sans min-h-screen text-left">
              <div className="border-b-4 border-blue-600 pb-6 mb-8 flex justify-between items-start">
                ...
                <div className="text-right text-xs">
                  <p className="font-bold">Reg Number: <span className="text-blue-600 font-extrabold">{selectedPrescription.doctorRegNo}</span></p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 print:hidden">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-start">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-black text-emerald-400 uppercase tracking-widest">Verified Rx</span>
                      </div>
                    ...
                    <div>
                      ...
                      <p className="text-[9px] font-black text-blue-500 uppercase tracking-widest mt-0.5">Clinical Authorization</p>
                    </div>

                    <div className="space-y-2 pt-2">
                      <p className="text-[10px] font-bold text-zinc-500 flex justify-between">
                        <span>Medicines:</span>
                        <span className="text-zinc-300 font-black">{p.medicines.map(m => m.name).join(', ')}</span>
                      </p>
                      <p className="text-[10px] font-bold text-zinc-500 flex justify-between">
                        <span>Issued Reg:</span>
              ...
              {filteredPrescriptions.length === 0 && (
                <div className="col-span-full py-16 text-center bg-zinc-950/80 border border-white/5 border-dashed rounded-[40px]">
                  ...
                  <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">No prescription records found in your folder.</p>
                </div>
              )}
            </div>
          )}
              ...
              <div className="bg-white text-zinc-800 p-8 rounded-2xl shadow-xl space-y-6 font-sans">
                ...
                <div className="grid grid-cols-2 gap-4 text-[10px] border-b border-zinc-150 pb-4">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-zinc-800 mt-1">Patient Node</p>
                        ...
                        <tr key={idx}>
                          <td className="py-3">
                            ...
                            <button
                              ...
                              className="mt-3 px-3 py-1.5 bg-blue-600 text-white text-[7px] font-black uppercase tracking-widest rounded-lg flex items-center gap-1 hover:bg-blue-500 transition-all active:scale-95"
                            >
                              <Plus size={8} strokeWidth={4} /> Add to Schedule
                            </button>
                          </td>
                          <td className="py-3">{med.dosage}</td>
                          <td className="py-3 uppercase">{med.frequency}</td>
                ...
                <div className="border-t border-zinc-150 pt-4 flex justify-between items-center text-[9px] text-zinc-400">
                  ...
                  <p>Digital Stamp: AUTHORIZED_BY_MEDI_CONSULT_CLINIC</p>
                </div>
              </div>
            </div>
          </div>
